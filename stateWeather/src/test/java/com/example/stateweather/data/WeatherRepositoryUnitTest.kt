package com.example.stateweather.data
...
class WeatherRepositoryUnitTest {
    private val weatherApiService: com.example.stateweather.data.remote.WeatherApiService = mockk()

    @OptIn(ExperimentalCoroutinesApi::class)
    private val testDispatcher = UnconfinedTestDispatcher()

    private lateinit var weatherRepository: com.example.stateweather.domain.repository.WeatherRepository

    @OptIn(ExperimentalCoroutinesApi::class)
    @Before
    fun setUp() {
        Dispatchers.setMain(testDispatcher)
        weatherRepository = com.example.stateweather.data.repository.WeatherRepositoryImpl(
            weatherApiService,
            testDispatcher
        )
    }
    ...
    fun `getStateWeather Return State Weather Successfully`() = runTest {
        ...
        val expectedWeather = mockk<com.example.stateweather.data.remote.dto.StateWeatherDto>()
